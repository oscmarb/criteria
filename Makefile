...
build:
		docker build -t criteria .

erase: ## Remove the Docker containers of the project.
		docker rmi criteria

composer-install: ## Install composer dependencies
		docker run --rm -v .:/app criteria composer install

composer-update: ## Update composer dependencies
		docker run --rm -v .:/app criteria composer update

phpunit: ## Run PHPUnit
		docker run --rm -v .:/app criteria vendor/bin/phpunit tests
